...
export default class ModuleLoader {
	...
	private async updateInteractions(commands: Collection<string, ClientCommand>) {
		const mainGuild = await client.guilds.fetch(config.guildId);

		await client.application!.commands.fetch({ guildId: mainGuild.id });

		let activeCommands = await mainGuild.commands.set(commands.map(c => c.command));
